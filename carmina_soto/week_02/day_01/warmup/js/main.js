...
const createCipher = function (message){
  const alphabet = 'abcdefghijklmnopqrstuvwxyz,.?!';
  let result = '';
  for (let i = 0; i < message.length; i++){
    const letterIndex = alphabet.indexOf(message[i].toLowerCase());
    if(letterIndex !== -1){
      result += (letterIndex % 5) + 1;
      result += Math.floor(letterIndex / 6) + 1;
      result += ' ';
    }
  }
  return result;
...
console.log(createCipher('message'));
