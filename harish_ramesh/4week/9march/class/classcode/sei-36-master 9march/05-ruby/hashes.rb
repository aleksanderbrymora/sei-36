...
users["Jorge"] = { :twitter => 'edwardawkward', :favorite_numbers => [666, 69, 420, 12] }
p users
...
# p users["Jonathan"][:favorite_numbers] & users["Anil"][:favorite_numbers] & users["Erik"][:favorite_numbers] & users["Jorge"][:favorite_numbers]
...
p users.values.map { |u| u[:favorite_numbers] }.flatten.uniq.sort
binding.pry
