...
def stops_lister (start_line,origin,end_line,destination)
  if start_line == end_line
    ...
    puts "stopping at #{$stopping_at.join(', ')} and no. of stops:#{$stopping_at.size}"
  else
    $stopping_at = stops_fetcher(origin,$us,start_line)
    $stops_after_change = stops_fetcher($us,destination,end_line)

    puts "stopping at #{$stopping_at.join(', ')} change at #{$us} journey continues through #{$stops_after_change.join(', ')} and no. of stops:#{$stopping_at.size + $stops_after_change.size}"
  end
end
#########################################################
